...
if __name__ == '__main__':
    ...
    num_class=4
    bs=4
    #实现细节：将batch作为shot的载体 sup的bs是num_class*num_support que的bs是num_class*num_query
    sup=torch.randn(16,3,224,224).to(get_device())
    que=torch.randn(4,3,224,224).to(get_device())
    dcn.to(get_device())
    x=dcn(sup,que)
